...
function PayloadMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, theme = 'core', drawnPolygons = [], arcgisLayers = [], drawMode = null, onDrawComplete, onDrawProgress, onDrawCancel, drawCommand = null, onMapCenter, route = null, userLocation = null, followUser = false, onFollowInterrupt, navigating = false }: PayloadMapProps) {
  ...
  useEffect(() => {
    ...
    for (const canvasContextAttributes of attributeFallbacks) {
      try {
        ...
        console.warn('[Payload Terminal] WebGL context rejected, retrying with weaker attributes:', e instanceof Error ? e.message : e);
      }
    }
    if (!map) return;
    ...
    map.on('load', () => {
      ...
      // Create icons — Payload Terminal Unified Palette
      ...
      // ══ Payload Terminal SDK — Lattice Intelligence Mesh ══
    ...
    // ── Payload Terminal SDK link click ──
      ...
      map.on('click', layer, e => {
        ...
        popup([coords.lng, coords.lat], `<div style="${pStyle}border:1px solid ${domainColor}40;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:9px;margin-bottom:8px;">
            ...
            <div><span style="color:#5C5A54;">SOURCE</span><br/><a href="${urlSafe(srcUrl)}" target="_blank" style="color:${domainColor};text-decoration:underline;cursor:pointer;">${htmlEsc(p.source || 'Payload Terminal')}</a></div>
          </div>
  ...
  // ══ Payload Terminal SDK — Lattice Sensor Mesh ══
  ...
  useEffect(() => {
    ...
    try {
      if (enabled) {
        ...
        if (!map.getSource('payload-buildings')) {
          map.addSource('payload-buildings', {
            type: 'vector',
        ...
        if (!map.getLayer('payload-3d-buildings')) {
          map.addLayer({
            id: 'payload-3d-buildings',
            source: 'payload-buildings',
        ...
        // ── DISABLE 3D ──
        if (map.getLayer('payload-3d-buildings')) map.removeLayer('payload-3d-buildings');
      }
    } catch (e) {
      console.warn('[Payload Terminal] 3D terrain toggle error:', e);
    }
...
export default memo(PayloadMap);
