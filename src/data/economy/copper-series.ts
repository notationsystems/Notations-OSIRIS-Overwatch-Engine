...
export const COPPER_SERIES_OBSERVATIONS: Observation[] = [
  ...Object.entries(MINE_SERIES).flatMap(([entityId, values]) =>
    values.map((value, i): Observation => {
      ...
      return {
        ...
        entityId, metric: 'production', value, unit: 'kt/y', basis: 'metal_content', period: yearPeriod(year),
  ...
  ...PANAMA_SERIES.map(([year, value]): Observation => ({
    id: `obs:prod:pa:${year}`,
    entityId: 'ent:country:pa', metric: 'production', value, unit: 'kt/y', basis: 'metal_content', period: yearPeriod(year),
    knownAt: `${year + 1}-01-31`,
  ...
  ...CN_REFINED_SERIES.map((value, i): Observation => {
    ...
    return {
      ...
      entityId: 'ent:country:cn', metric: 'refined_production', value, unit: 'kt/y', basis: 'metal_content', period: yearPeriod(year),
