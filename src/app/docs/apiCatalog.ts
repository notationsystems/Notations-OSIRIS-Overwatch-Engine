...
export const API_GROUPS: ApiGroup[] = [
  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        notes: 'Requires query authority. The view is derived from the real canonical corpus, public projection, persistent agent journal, source identities, MCP tools, Kepler adapter, build attestation and pinned operational-event SP1 identity. It reports capability mode, approval, health, provenance, timeline reason/requesting role and dispatched=false. Missing cost, latency, spatial, source-health, and prover-runtime observations stay explicitly unobserved.',
        env: ['PAYLOAD_CORPUS_QUERY_TOKEN', 'PAYLOAD_CORPUS_DATABASE_PATH', 'PAYLOAD_CORPUS_READ_MODEL_PATH', 'PAYLOAD_CORPUS_AGENT_ARTIFACT_PATH', 'PAYLOAD_CORPUS_CONTROL_STALE_AFTER_MS'],
        requiresAuth: true,
      },
      {
