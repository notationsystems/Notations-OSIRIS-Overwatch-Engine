...
export const API_GROUPS: ApiGroup[] = [
  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        method: ['GET', 'POST'],
        summary: 'Builds and persists an agent context, or retrieves the exact saved result by resultId.',
        params: [{ name: 'resultId', desc: 'GET-only immutable corpus-result: identity.' }],
        returns: ['kind', 'resultId', 'plan', 'context', 'agentContext', 'spatial', 'persistence'],
        notes: 'Requires dedicated query authority. Reads only the current verified public projection, bounds resolution/traversal/evidence retrieval, refuses stale or unavailable historical state, and returns exact record IDs and CorpusBuild identity in its trace. mode=plan omits execution; mode=agent accepts FAST, GROUNDED, AUDIT, or VERIFIED and appends the response to the hash-chained artifact journal. Higher evidence budgets add detail without changing assertion identity. The spatial envelope carries OGC:CRS84 GeoJSON and a kepler.gl addDataToMap payload. VERIFIED includes checked CorpusBuild membership proofs; an exact stored Ed25519 build signature elevates it to ATTESTED, while corpus SP1 proof remains absent.',
        env: ['PAYLOAD_CORPUS_DATABASE_PATH', 'PAYLOAD_CORPUS_QUERY_DATABASE_URL', 'PAYLOAD_CORPUS_READ_MODEL_PATH', 'PAYLOAD_CORPUS_AGENT_ARTIFACT_PATH', 'PAYLOAD_CORPUS_QUERY_TOKEN'],
        requiresAuth: true,
      ...
      {
        path: '/api/corpus/attestations',
        method: ['GET', 'POST'],
        summary: 'Signs the current CorpusBuild commitment or retrieves its latest immutable attestation.',
        params: [{ name: 'corpusBuildId', desc: 'Exact build identity; required on GET and optional as a POST mismatch guard.' }],
        returns: ['kind', 'attestation', 'persistence', 'zkPrograms'],
        notes: 'POST requires compiler authority, a current non-stale public projection, persistent artifact storage, and a protected Ed25519 PEM. GET requires query authority. The signature authenticates the exact build commitment but names its time basis as SIGNER_CLOCK and does not claim empirical truth or independent timestamping. The returned ceremonially pinned SP1 event-batch identity is explicitly marked as not applicable to corpus builds.',
        env: ['PAYLOAD_CORPUS_COMPILER_TOKEN', 'PAYLOAD_CORPUS_QUERY_TOKEN', 'PAYLOAD_CORPUS_ATTESTATION_PRIVATE_KEY_PATH', 'PAYLOAD_CORPUS_AGENT_ARTIFACT_PATH'],
        requiresAuth: true,
        bodyExample: '{\n  "corpusBuildId": "corpus-build:...",\n  "signedAt": "2026-09-02T12:00:00.000Z"\n}',
      },
      {
        path: '/api/corpus/control-plane',
        method: ['GET'],
        summary: 'Inspects the live Payload physical-economy stack and its operator attention state.',
        params: [
          { name: 'afterSequence', desc: 'Exclusive immutable agent-artifact event cursor.', example: '0' },
          { name: 'limit', desc: 'Timeline page size from 1 to 500.', example: '100' },
        ],
        returns: ['revision', 'topology', 'timeline', 'dock', 'operator'],
        notes: 'Requires query authority. The view is derived from the real canonical corpus, public projection, build-bound index, Notation federation and destination state, persistent agent journal, source identities, MCP tools, Kepler adapter, build attestation and pinned operational-event SP1 identity. It reports capability mode, approval, health, provenance, timeline reason/requesting role and dispatched=false. Destination cursor and acknowledgement lag are observed; unavailable cost, latency, spatial, source-health, and prover-runtime observations stay explicitly unobserved.',
        env: ['PAYLOAD_CORPUS_QUERY_TOKEN', 'PAYLOAD_CORPUS_DATABASE_PATH', 'PAYLOAD_CORPUS_READ_MODEL_PATH', 'PAYLOAD_CORPUS_INDEX_PATH', 'PAYLOAD_CORPUS_AGENT_ARTIFACT_PATH', 'PAYLOAD_NOTATION_SUBSTRATE_DATABASE_PATH', 'PAYLOAD_CORPUS_CONTROL_STALE_AFTER_MS'],
        requiresAuth: true,
      },
      {
        path: '/api/corpus/index',
        method: ['GET', 'POST'],
        summary: 'Builds or searches a deterministic lexical/faceted index over the exact current public CorpusBuild.',
        params: [
          { name: 'q', desc: 'Lexical query; required for the default search view.', example: 'polymer terminal' },
          { name: 'view', desc: 'search, manifest, or coverage.', example: 'coverage' },
          { name: 'recordTypes', desc: 'Comma-separated canonical record types.' },
          { name: 'entityKinds', desc: 'Comma-separated physical-economy entity kinds.' },
          { name: 'predicates', desc: 'Comma-separated relationship predicates.' },
          { name: 'sourceIds', desc: 'Comma-separated source identities.' },
          { name: 'asOf', desc: 'Optional valid-time ceiling.' },
          { name: 'knownAt', desc: 'Optional knowledge-time ceiling.' },
          { name: 'west,south,east,north', desc: 'Complete OGC:CRS84 bounding box.' },
          { name: 'limit', desc: 'Search result limit from 1 to 100.', example: '25' },
        ],
        returns: ['indexId', 'corpusBuildId', 'projectionDigest', 'hits', 'coverage', 'limitations'],
        notes: 'POST requires compiler authority; GET requires query authority. The store is disposable SQLite/WAL and is refused if its CorpusBuild or projection digest is stale. Scores are lexical relevance, not confidence or truth; missing coverage does not establish physical-world absence.',
        env: ['PAYLOAD_CORPUS_COMPILER_TOKEN', 'PAYLOAD_CORPUS_QUERY_TOKEN', 'PAYLOAD_CORPUS_READ_MODEL_PATH', 'PAYLOAD_CORPUS_INDEX_PATH'],
        requiresAuth: true,
        bodyExample: '{\n  "builtAt": "2026-09-02T12:00:00.000Z"\n}',
      },
      {
        path: '/api/corpus/federation',
        method: ['GET', 'POST'],
        summary: 'Synchronizes the current public Payload CorpusBuild into the Notation Data Substrate identity space.',
        params: [
          { name: 'channel', desc: 'payload:public:global (ready), internal-global, or customer:<tenant>; non-public channels remain fail-closed until separately projected.' },
          { name: 'afterSequence', desc: 'Exclusive canonical sequence cursor.', example: '0' },
          { name: 'limit', desc: 'Sync page size from 1 to 500.', example: '100' },
        ],
        returns: ['sourceNodeUri', 'corpusBuildId', 'projectionDigest', 'nextAfterSequence', 'hasMore', 'envelopes', 'pageDigest'],
        notes: 'Requires projection-worker authority. GET emits deterministic notation:// envelopes from the exact current policy-filtered public/global CorpusBuild. POST advances a named consumer checkpoint only when build and projection identities match. Payload remains canonical. Internal/global and customer channel contracts declare separate entitlement and policy requirements and refuse until those projections exist.',
        env: ['PAYLOAD_CORPUS_PROJECTOR_TOKEN', 'PAYLOAD_CORPUS_PROJECTOR_DATABASE_URL', 'PAYLOAD_CORPUS_READ_MODEL_PATH'],
        requiresAuth: true,
        bodyExample: '{\n  "consumerId": "primary-fabric",\n  "sequence": 1204,\n  "corpusBuildId": "corpus-build:...",\n  "projectionDigest": "..."\n}',
      },
      {
        path: '/api/corpus/substrate',
        method: ['GET', 'POST'],
        summary: 'Inspects the durable federation destination and writes or searches exact model/version vector projections.',
        params: [
          { name: 'view', desc: 'status, acknowledgements, lag, or semantic-documents.', example: 'status' },
          { name: 'afterSequence', desc: 'Acknowledgement journal cursor.' },
          { name: 'channelId', desc: 'Optional lag channel filter.', example: 'payload:public:global' },
          { name: 'withoutModelId,withoutModelVersion', desc: 'Return semantic documents missing exactly this model/version projection.' },
          { name: 'limit', desc: 'Bounded page size.' },
        ],
        returns: ['counts', 'channels', 'vectorModels', 'boundaries', 'acknowledgements', 'samples', 'documents', 'hits'],
        notes: 'GET and vector_search require query authority; put_vector requires projector authority. The store verifies source envelopes and global identities before mutation, keeps a hash-chained acknowledgement journal and immutable lag samples, and binds every vector to an exact semantic-document hash plus model/version. Cosine similarity is not truth, confidence, causality, or materiality.',
        env: ['PAYLOAD_NOTATION_SUBSTRATE_DATABASE_PATH', 'PAYLOAD_CORPUS_QUERY_TOKEN', 'PAYLOAD_CORPUS_PROJECTOR_TOKEN'],
        requiresAuth: true,
        bodyExample: '{\n  "action": "put_vector",\n  "documentUri": "notation://state/payload/semantic-document%3Arecord%3Aentity%3A123",\n  "documentHash": "...",\n  "modelId": "embedding:provider",\n  "modelVersion": "1.0.0",\n  "values": [0.12, -0.04]\n}',
      },
      {
