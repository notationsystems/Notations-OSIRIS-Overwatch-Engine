...
|---|---|
| `concentration` | HHI of mine production (country/mine/**operator**), refined production, consumption, smelting/refining capacity — each from the latest observation per entity at `asOf`. DOJ bands (<1500 / 1500–2500 / >2500). Never mixes entity kinds in one calculation. **Every index carries its partition context** (`groupCount`, `effectiveGroups` = 10000/HHI, `partitionFloor` = 10000/n): HHI has a floor of 10000/n, so a finer partition yields a lower index whatever the structure, and raw HHIs over unequal partitions are never comparable. Operator concentration ships in **two attribution bases, stated on the number**: `control` (100% of an asset to its operator of record — who can stop it; JV-operated facilities with no modeled operator fall to the reported unattributed remainder) and `economic_interest` (ownership shares — who owns the loss). Measured, fully labeled: over the modeled facility universe, control-HHI **1778** (9 operators, effective 5.6, 84.5% attribution), economic-HHI 959 (15 holders, effective 10.4); the same facility universe grouped by country gives 2135 (9 countries, effective 4.7 — the one strictly comparable geographic figure, itself inflated by Chile-heavy facility coverage); the world-reported country figure is 1339 over 19 reporters (different universe — labeled, not compared raw). The index-free finding is unaffected by all of it: **Freeport-McMoRan controls 31.2% of modeled mine output (1,680 kt) across Indonesia, Peru and the US** — a single-operator exposure the country lens scores as three unrelated events, expressible as a company-level scenario reaching every operated asset. Shareholder edges never traverse: a Rio Tinto event cannot reach Escondida through its 30%. |
